...
var ProviderService = /** @class */ (function () {
    function ProviderService() {
        this.data = null;
        this.objectStorageAccess = null;
        this.recommendationEngineAccess = null;
        this.VisualRecognitionAccess = null;
        console.log('--> Mobile Foundation: constructor() called');
    }
    ProviderService.prototype.getObjectStorageAccess = function () {
        var _this = this;
        console.log('--> Mobile Foundation: getting Object Storage AuthToken from adapter ...');
        return new Promise(function (resolve, reject) {
            if (_this.objectStorageAccess) {
                // already loaded data
                return resolve(_this.objectStorageAccess);
            }
            var dataRequest = new WLResourceRequest("/adapters/VisualRecognition/resource/objectStorage", WLResourceRequest.GET);
            dataRequest.send().then(function (response) {
                console.log('--> Mobile Foundation: got Object Storage AuthToken from adapter ', response);
                _this.objectStorageAccess = response.responseJSON;
                resolve(_this.objectStorageAccess);
            }, function (failure) {
    ...
    ProviderService.prototype.getRecommendationEngineAccess = function () {
        var _this = this;
        console.log('--> Mobile Foundation: getting Recommendation Engine API from adapter ...');
        return new Promise(function (resolve, reject) {
            if (_this.recommendationEngineAccess) {
                // already loaded data
                return resolve(_this.recommendationEngineAccess);
            }
            var dataRequest = new WLResourceRequest("/adapters/VisualRecognition/resource/recommendationEngine", WLResourceRequest.GET);
            dataRequest.send().then(function (response) {
                console.log('--> Mobile Foundation: got Recommendation Engine API from adapter ', response);
                _this.recommendationEngineAccess = response.responseJSON;
                resolve(_this.recommendationEngineAccess);
            }, function (failure) {
    ...
    ProviderService.prototype.getVisualRecognitionAccess = function () {
        var _this = this;
        console.log('--> Mobile Foundation: getting Visual Recognition API from adapter ...');
        return new Promise(function (resolve, reject) {
            if (_this.VisualRecognitionAccess) {
                // already loaded data
                return resolve(_this.VisualRecognitionAccess);
            }
            var dataRequest = new WLResourceRequest("/adapters/VisualRecognition/resource/visualRecognition", WLResourceRequest.GET);
            dataRequest.send().then(function (response) {
                console.log('--> Mobile Foundation: got Visual Recognition API from adapter ', response);
                _this.VisualRecognitionAccess = response.responseJSON;
                resolve(_this.VisualRecognitionAccess);
            }, function (failure) {
